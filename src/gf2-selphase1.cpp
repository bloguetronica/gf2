/* GF2 SelPhase1 Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == GF2Device::SUCCESS) {  // Device was successfully opened
        int errcnt = 0;
        std::string errstr;
        device.selectPhase(GF2Device::PSEL1, errcnt, errstr);  // Select phase 1 as active
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "Phase 1 selected." << std::endl;
        }
        device.close();

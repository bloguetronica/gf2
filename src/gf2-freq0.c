/* GF2 Freq0 Command - Version 1.0 for Debian Linux
...
void set_freq0(libusb_device_handle *devhandle, unsigned int value);
        ...
        else  // If libusb is initialized
        {
            ...
            else  // If the device is successfully opened and a handle obtained
            {
                ...
                else  // If the interface is successfully claimed
                {
                    ...
                    set_freq0(devhandle, freq_code);  // Set the FREQ0 register to the intended value (by sending a sequence of bytes to the AD9834 waveform generator on channel 0)
                    ...
                    if (err_level == 0)  // If all goes well
                        printf("Frequency 0 set to %.4fKHz.\n", freq_code * 40000.0 / 134217728);
...
void set_freq0(libusb_device_handle *devhandle, unsigned int value)  // Sets the FREQ0 register on the AD9834 waveform generator to a given value
{
    unsigned char write_command_buf[12] = {
        ...
        0x40 + (0x3F & (unsigned char)(value >> 8)),  // FREQ0 register set to the given value
        (unsigned char)value,
        0x40 + (0x3F & (unsigned char)(value >> 22)),

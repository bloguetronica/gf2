/* GF2 Freq1 Command - Version 2.0 for Debian Linux
    ...
    if (argc < 2) {  // If the program was called without arguments
        ...
        if (frequency < GF2Device::FREQUENCY_MIN || frequency > GF2Device::FREQUENCY_MAX) {  // If the obtained frequency value (in KHz) after conversion is lesser than "FREQUENCY_MIN" [0] or greater than "FREQUENCY_MAX" [40000]
            ...
            if (err == GF2Device::SUCCESS) {  // Device was successfully opened
                ...
                device.setFrequency(GF2Device::FSEL1, frequency, errcnt, errstr);  // Set frequency 1 to the given value
                if (errcnt > 0) {  // In case of error
                    ...
                    std::cout << "Frequency 1 set to " << std::fixed << std::setprecision(4) << GF2Device::expectedFrequency(frequency) << "KHz." << std::endl;  // Print the set frequency
                }
                device.close();

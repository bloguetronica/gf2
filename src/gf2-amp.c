/* GF2 Amp Command - Version 1.0 for Debian Linux
...
void set_amplitude(libusb_device_handle *devhandle, unsigned short value);
...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        if (amplitude < 0 || amplitude > 8)  // If the obtained amplitude value (in Vpp) after conversion is lesser than 0 or greater than 8
        {
            fprintf(stderr, "Error: Amplitude should be between 0 and 8Vpp.\n");
            err_level = EXIT_USERERR;
        }
        ...
        else  // If libusb is initialized
        {
            ...
            else  // If the device is successfully opened and a handle obtained
            {
                ...
                {
                    amp_code = (unsigned short)(amplitude * 1023 / 8 + 0.5);
                    ...
                    set_amplitude(devhandle, amp_code);  // Set the amplitude to the intended value (by sending a byte containing said value to the AD5310 DAC on channel 1)
...
void set_amplitude(libusb_device_handle *devhandle, unsigned short value)  // Sets the register on the AD5310 DAC to a given value, in order to set the amplitude (channel 1 must be enabled)
{
    unsigned char write_command_buf[10] = {
        ...
        0x0F & (unsigned char)(value >> 6),  // Amplitude set according to the given value

/* GF2 Start Command - Version 1.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    else  // If libusb is initialized
    {
        ...
        else  // If the device is successfully opened and a handle obtained
        {
            ...
            {
                set_gpio2(devhandle, true);  // Set GPIO.2 to a logical high to enable reset, thus enforcing a restart if the waveform generator is already running
                set_gpio2(devhandle, false);  // Set GPIO.2 to a logical low in order to disable reset and (re)start the waveform generation

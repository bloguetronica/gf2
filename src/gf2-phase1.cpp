/* GF2 Phase1 Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {  // If the program was called without arguments
        ...
        if (err == GF2Device::SUCCESS) {  // Device was successfully opened
            ...
            device.setPhase(GF2Device::PSEL1, phase, errcnt, errstr);  // Set phase 1 to the given value
            if (errcnt > 0) {  // In case of error
                ...
                std::cout << "Phase 1 set to " << std::fixed << std::setprecision(2) << GF2Device::expectedPhase(phase) << "°." << std::endl;  // Print the set phase
            }
            device.close();

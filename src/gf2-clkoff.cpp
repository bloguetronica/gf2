/* GF2 ClkOff Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == GF2Device::SUCCESS) {  // Device was successfully opened
        int errcnt = 0;
        std::string errstr;
        device.setClockEnabled(false, errcnt, errstr);  // Disable the synchronous clock
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "Synchronous clock disabled." << std::endl;
        }
        device.close();

/* GF2 Freq Command - Version 2.0 for Debian Linux
    ...
    if (argc < 2) {  // If the program was called without arguments
        ...
        if (frequency < GF2Device::FREQUENCY_MIN || frequency > GF2Device::FREQUENCY_MAX) {  // If the obtained frequency value (in KHz) after conversion is lesser than "FREQUENCY_MIN" [0] or greater than "FREQUENCY_MAX" [40000]
            ...
            if (err == GF2Device::SUCCESS) {  // Device was successfully opened
                int errcnt = 0;
                std::string errstr;
                bool fsel = device.getFrequencySelection(errcnt, errstr);  // Get the current frequency selection
                device.setupChannel0(errcnt, errstr);  // Setup channel 0
                device.setFrequency(fsel, frequency, errcnt, errstr);  // Set the active frequency (frequency 0 or frequency 1, whichever is selected) to the given value
                if (errcnt > 0) {  // In case of error
                    ...
                    std::cout << "Frequency (" << fsel << ") set to " << std::fixed << std::setprecision(4) << GF2Device::expectedFrequency(frequency) << "KHz." << std::endl;  // Print the set frequency
                }
                device.close();

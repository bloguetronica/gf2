/* GF2 Phase Command - Version 1.0 for Debian Linux
...
void set_selphase(libusb_device_handle *devhandle, unsigned short value, bool sel);
...
int main(int argc, char **argv)
{
    ...
    bool kernel_attached = false, psel;
    float phase;
        ...
        else  // If libusb is initialized
        {
            ...
            else  // If the device is successfully opened and a handle obtained
            {
                ...
                else  // If the interface is successfully claimed
                {
                    ...
                    set_selphase(devhandle, phase_code, psel);  // Set the selected phase register to the intended value (by sending a sequence of bytes to the AD9834 waveform generator on channel 0)
                    ...
                    if (err_level == 0)  // If all goes well
                        printf("Phase (%d) set to %.2f°.\n", psel, phase_code * 360.0 / 4096);
...
void set_selphase(libusb_device_handle *devhandle, unsigned short value, bool sel)  // Sets either PHASE0 or PHASE1 registers on the AD9834 waveform generator to the given value
{
    unsigned char write_command_buf[10] = {
        ...
        (sel ? 0xE0 : 0xC0) + (0x0F & (unsigned char)(value >> 8)),  // PHASE0 or PHASE1 register set to the given value, according to the boolean variable "sel"
        (unsigned char)value
    };
    int bytes_written;

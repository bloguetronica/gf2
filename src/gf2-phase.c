/* GF2 Phase Command - Version 1.1 for Debian Linux
        ...
        else  // If libusb is initialized
        {
            ...
            else  // If the device is successfully opened and a handle obtained
            {
                ...
                else  // If the interface is successfully claimed
                {
                    ...
                    float phase_mod = (float)fmod(atof(argv[1]), 360);  // Convert the argument string into a floating point number, and then calculate the remainder of its division by 360
                    uint16_t phase_code = (uint16_t)((phase_mod < 0 ? 360 + phase_mod : phase_mod) * 4096 / 360 + 0.5);
                    ...
                    set_phase(devhandle, phase_code, psel);  // Set the selected phase register to the intended value (by sending a sequence of bytes to the AD9834 waveform generator on channel 0)

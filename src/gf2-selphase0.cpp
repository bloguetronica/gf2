/* GF2 SelPhase0 Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == GF2Device::SUCCESS) {  // Device was successfully opened
        int errcnt = 0;
        std::string errstr;
        device.selectPhase(GF2Device::PSEL0, errcnt, errstr);  // Select phase 0 as active
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "Phase 0 selected." << std::endl;
        }
        device.close();

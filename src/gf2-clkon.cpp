/* GF2 ClkOn Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == GF2Device::SUCCESS) {  // Device was successfully opened
        int errcnt = 0;
        std::string errstr;
        device.setClockEnabled(true, errcnt, errstr);  // Enable the synchronous clock
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "Synchronous clock enabled." << std::endl;
        }
        device.close();

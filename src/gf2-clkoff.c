/* GF2 ClkOff Command - Version 1.1 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    else  // If libusb is initialized
    {
        ...
        else  // If the device is successfully opened and a handle obtained
        {
            ...
            {
                set_gpio6(devhandle, true);  // Set GPIO.6 to a logical high to disable the synchronous clock generation (TLV3501 comparator)
                if (err_level == 0)  // If all goes well
                    printf("Synchronous clock disabled.\n");

/* GF2 Amp Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {  // If the program was called without arguments
        ...
        float amplitude = std::atof(argv[1]);  // Convert the argument string into a floating point number
        if (amplitude < GF2Device::AMPLITUDE_MIN || amplitude > GF2Device::AMPLITUDE_MAX) {  // If the obtained amplitude value (in Vpp) after conversion is lesser than "AMPLITUDE_MIN" [0] or greater than "AMPLITUDE_MAX" [8]
            std::cerr << "Error: Amplitude should be between 0 and 8Vpp.\n";
            errlvl = EXIT_USERERR;
        } else {
            GF2Device device;
            ...
            if (err == GF2Device::SUCCESS) {  // Device was successfully opened
                ...
                if (errcnt > 0) {  // In case of error
                    ...
                    std::cout << "Amplitude set to " << std::fixed << std::setprecision(2) << GF2Device::expectedAmplitude(amplitude) << "Vpp." << std::endl;  // Print the set amplitude
                }
                device.close();

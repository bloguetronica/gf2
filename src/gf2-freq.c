/* GF2 Freq Command - Version 1.0 for Debian Linux
...
void set_selfreq(libusb_device_handle *devhandle, unsigned int value, bool sel);
...
int main(int argc, char **argv)
{
    ...
    bool fsel, kernel_attached = false;
    float frequency;
    ...
    else
    {
        ...
        else  // If libusb is initialized
        {
            ...
            else  // If the device is successfully opened and a handle obtained
            {
                ...
                {
                    freq_code = (unsigned int)(frequency * 134217728 / 40000 + 0.5);
                    ...
                    set_selfreq(devhandle, freq_code, fsel);  // Set the selected freq register to the intended value (by sending a sequence of bytes to the AD9834 waveform generator on channel 0)
...
void set_selfreq(libusb_device_handle *devhandle, unsigned int value, bool sel)  // Sets either FREQ0 or FREQ1 registers on the AD9834 waveform generator to the given value
{
    unsigned char write_command_buf[12] = {
        ...
        (sel ? 0x80 : 0x40) + (0x3F & (unsigned char)(value >> 8)),  // FREQ0 or FREQ1 register set to the given value, according to the boolean variable "sel"
        (unsigned char)value,
        (sel ? 0x80 : 0x40) + (0x3F & (unsigned char)(value >> 22)),

/* GF2 DACOn Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == GF2Device::SUCCESS) {  // Device was successfully opened
        int errcnt = 0;
        std::string errstr;
        device.setDACEnabled(true, errcnt, errstr);  // Enable the DAC that is internal to the AD9834 waveform generator
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "Waveform generator DAC enabled." << std::endl;
        }
        device.close();

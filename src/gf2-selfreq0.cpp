/* GF2 SelFreq0 Command - Version 2.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (err == GF2Device::SUCCESS) {  // Device was successfully opened
        int errcnt = 0;
        std::string errstr;
        device.selectFrequency(GF2Device::FSEL0, errcnt, errstr);  // Select frequency 0 as active
        if (errcnt > 0) {  // In case of error
            ...
            std::cout << "Frequency 0 selected." << std::endl;
        }
        device.close();
